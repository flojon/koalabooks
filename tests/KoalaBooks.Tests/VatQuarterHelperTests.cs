...
public class VatQuarterHelperTests
{
        ...
        int quarter, int ey, int em, int ed, int ey2, int em2, int ed2)
    {
        ...
        var (from, to) = VatQuarterHelper.ComputeRange(fy, quarter);

        Assert.Equal(new DateOnly(ey, em, ed), from);
        Assert.Equal(new DateOnly(ey2, em2, ed2), to);
    }
    ...
    public void ComputeRange_PartialFY_K4_ClampedToFYStart()
    {
        ...
        var (from, to) = VatQuarterHelper.ComputeRange(fy, 4);

        Assert.Equal(new DateOnly(2025, 11, 1), from); // clamped from Oct 1
    ...
    [InlineData(2)]
    [InlineData(3)]
    public void ComputeRange_PartialFY_OutOfRangeQuarters_ReturnEmptyRange(int quarter)
    {
        var fy = Fy(new DateOnly(2025, 11, 1), new DateOnly(2025, 12, 31));

        var (from, to) = VatQuarterHelper.ComputeRange(fy, quarter);

        // from == to signals an empty (not wrong) date range
    ...
    public void ComputeRange_BrokenFY_QuarterStraddlesFYStart_ClampedCorrectly()
    {
        ...
        (from, to) = VatQuarterHelper.ComputeRange(fy, 3);
