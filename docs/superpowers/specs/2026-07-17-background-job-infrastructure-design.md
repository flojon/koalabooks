...
A composite index on `(OrganisationId, JobType, Status)` is required — it's the shape of the "open/unacknowledged runs for this org+JobType" query that `IBackgroundJobRunService` runs, and that the poller (#5) hits on every 5s tick for every open job on every visible page.
