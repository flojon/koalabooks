...
Jobs that call a tenant-scoped service or query a tenant-filtered table need this — `ZipImportJob` today, and, since PR #289, `DocumentExtractionJob` too (see below). Jobs have no `HttpContext`, so a DI-resolved `ICurrentUser` is always null and any tenant-scoped write/query would be rejected or filtered to nothing. `ZipImportJob` works around this today by hand: construct an `AppDbContext` bound to a mutable `LocalCurrentUser` with no org set yet, look up its batch row via `IgnoreQueryFilters()` (unaffected by the still-empty tenant), then set `tenant.OrganisationId` from the loaded row so every subsequent query/write on that same context is correctly scoped from that point on.
...
**Corrected 2026-07-18:** this section originally said `DocumentExtractionJob` needed no retrofit here because it "never calls a tenant-scoped service." That was true when this doc was written, but PR #289 (fixing #287's missing `HasQueryFilter` on `DocumentData`/`CustomerInvoiceLine`) invalidated it: `DbDocumentStorage`'s `DocumentData.FindAsync` lookup now respects that filter, so `DocumentExtractionJob`'s `storage.LoadAsync` call silently returned empty bytes under its old null-org DI-injected `AppDbContext`. PR #289 fixed this by hand-rolling the exact two-phase bootstrap above directly in `DocumentExtractionJob` (ahead of this helper existing on that branch), constructing its own `AppDbContext`/`DbDocumentStorage` instead of taking them via DI. `DocumentExtractionJob` still has no `BackgroundJobRun` row and doesn't need `BackgroundJobRunBase` (#3) — but it does need `JobTenantContext.CreateUnscoped` now that this helper exists, as a dedup of #289's hand-rolled copy, not a new behavior. See Sequencing.
...
Concrete jobs (`SieImportJob`, `BasImportJob`, `YearEndCloseJob`, `SieExportJob`, and the refactored `ZipImportJob`) inherit this and implement only their business logic in `RunAsync`. `DocumentExtractionJob` does not inherit `BackgroundJobRunBase` — it has no `BackgroundJobRun` row to manage — but per the correction in #2, it should be retrofitted onto `JobTenantContext.CreateUnscoped` directly (bypassing `BackgroundJobRunBase`'s run-lifecycle methods, which it has no use for).
...
3. `DocumentExtractionJob` needs no `BackgroundJobRun` and no `BackgroundJobRunBase`. It does need a small retrofit onto `JobTenantContext.CreateUnscoped` (see #2's correction): PR #289 already hand-rolled the equivalent bootstrap directly in `DocumentExtractionJob` (it predates this PR), so once this lands, swap that hand-rolled copy for a call to the new helper — pure dedup, no behavior change. Not blocking: `DocumentExtractionJob` works correctly either way, so this can land in the same PR as this shared infra or as a small immediate follow-up, whichever is simpler once #289 has actually merged.
