...
- Every Large Object read/write/unlink happens inside a transaction opened via `db.Database.BeginTransactionAsync()`, using `(NpgsqlConnection)db.Database.GetDbConnection()` as the connection for all raw commands in that method.
