...
Note (learned during Task 4's review): the exception only actually fires when an EF-Core-guarded operation (a query, or `SaveChangesAsync`) runs *while* the manual transaction is open — a wrapped delegate that only issues raw `NpgsqlCommand`s never touches that guard, wrap or no wrap. Concretely: `RunAsync`'s staging-copy delegate (`readStrategy.ExecuteAsync` in Step 3, around line 1217) only calls `PostgresLargeObjects.CopyLargeObjectIntoStreamAsync` — no `SaveChangesAsync` inside it — so reverting *that* wrap will NOT reproduce the exception, and is not a useful revert target for this test. `DeleteStagingAsync`'s wrap (`strategy.ExecuteAsync` in Step 3, around line 1305), by contrast, does call `db.SaveChangesAsync()` inside the delegate (setting `batch.StagingOid = null`) — reverting *that* one is what will actually reproduce the exception. The wrapping is still correct practice for both call sites (retry-as-a-unit, consistency with `DbDocumentStorage`'s reference pattern), but only `DeleteStagingAsync`'s wrap is empirically load-bearing for this specific test.
...
Expected: PASS. Then temporarily revert `DeleteStagingAsync`'s wrap specifically (remove `CreateExecutionStrategy().ExecuteAsync(...)` there, calling `db.Database.BeginTransactionAsync()` directly instead — do NOT use `RunAsync`'s staging-copy wrap for this, see the note above) and re-run this one test to confirm it fails with the `NpgsqlRetryingExecutionStrategy does not support user-initiated transactions` error — proving the test actually catches the regression — then restore the fix.
