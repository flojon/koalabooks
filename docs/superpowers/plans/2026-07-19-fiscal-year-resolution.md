...
- The REST endpoints added by PR #278 (`GET api/v1/fiscal-years/active` and the three `{fiscalYearId}`-scoped count endpoints) live only in the unmerged WASM worktree branch stack, not on `main` — out of scope for this plan; flag for whoever lands that branch that the count endpoints need organisation-scoped counterparts once this plan's Task 2 methods exist.
...
- The `GET api/v1/fiscal-years/active` REST endpoint and its three `{fiscalYearId}` count-endpoint siblings from PR #278 — they live only in the unmerged WASM worktree branch stack, not `main`. Once this plan lands, whoever picks that branch back up needs to: rename `/active` to `/default` backed by `GetDefaultFiscalYearAsync()`, and add organisation-scoped counterparts to the three count endpoints backed by this plan's Task 2 methods, so the WASM-rendered badges (once wired up) match the Server-rendered ones from Task 6.
