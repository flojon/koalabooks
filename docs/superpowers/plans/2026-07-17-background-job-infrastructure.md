...
- Also deferred, smaller: `DocumentExtractionJob` (see design doc §2, corrected 2026-07-18). PR #289 fixed a real cross-tenant regression by hand-rolling the exact bootstrap `JobTenantContext.CreateUnscoped` (Task 3) now provides, directly inside `DocumentExtractionJob` — done before this helper existed on that branch. Once #289 has merged, swap that hand-rolled copy for a call to `JobTenantContext.CreateUnscoped` as a small follow-up (dedup only, no behavior change, no `BackgroundJobRunBase` involved — `DocumentExtractionJob` still has no `BackgroundJobRun` row).
