...
- **Wiring `WasmClient:RedirectUri` to real per-environment config before any real page ships.** `WasmClientSeeder`'s default (`https://localhost:7154/authentication/login-callback`) only matches local dev; `AddOidcAuthentication` computes its actual redirect from `HostEnvironment.BaseAddress` (the real deployed origin), so these will mismatch in any other environment, causing an `invalid redirect_uri` on the first real authorize request. Harmless today since the route is unreachable — same operational requirement `AspireDashboard:OidcRedirectUri` already has, flagged during final review for when this stops being inert.
- **Re-auditing `AuthenticationStateSerializationOptions.SerializeAllClaims = true`** once a real WASM page exists: this serializes every cookie-principal claim into the prerendered page HTML sent to the browser. Confirm no sensitive claim rides along, or narrow to an explicit allowlist, before real components consume it.
