...
| `ICustomerInvoiceService` | Application | `CreateAsync`, `PostAsync`, `FindMatchingBankTransactionsAsync`, `MarkAsPaidAsync`, `DeleteAsync` — implemented. **No** `GetAllAsync`-by-fiscal-year list method or `CreateFromEntryAsync` confirmed — verify against `ICustomerInvoiceService.cs` before assuming list/from-entry endpoints are free |
...
| **Customer invoices** | list/by-id/create/from-entry/post/mark-paid/find-matching/delete/pdf | ⬜ (new controller) | `ICustomerInvoiceService` + `CustomerInvoicePdfGenerator.Generate` (verify list-by-fiscal-year and from-entry methods exist, see 5.E) |
...
**Open question (5.E):** the interface dumps I read show `ICustomerService` with no `GetByIdAsync`/`GetAllAsync`-signature confirmed only as `GetAllAsync(int organisationId)`, and `ICustomerInvoiceService` with no fiscal-year-scoped list method or `CreateFromEntryAsync`. Before writing controllers, Agent E must re-read both interfaces in full and either confirm these methods exist under different names or flag the gap back rather than adding methods to the service under the "no duplicate business logic in controllers" rule.
...
- Three genuine ambiguities were found by reading the code rather than assumed away: FiscalYears "close year" vs Year-end closing overlap (5.B), two `ICustomerService`/`ICustomerInvoiceService` methods not confirmed present in the interface dump (5.E), and the SIE-import/#279 async-shape cross-reference plus the missing `IBulkJournalImportService` (5.H). All four are flagged to the user rather than resolved by invention, per the task brief's explicit instruction.
