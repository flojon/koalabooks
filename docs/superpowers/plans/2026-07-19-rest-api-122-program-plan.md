...
- **PR #291** (`worktree-issue-256-wasm-review-poc` → `main`, currently **OPEN, not merged, `CONFLICTING` with `main`**) has PR #298 folded into it — a mechanical relocation of ~14 Application-layer service interfaces (`IAccountService`, `IFiscalYearService`, `ICustomerService`, `ICustomerInvoiceService`, `IJournalEntryService`, `IJournalEntryReportingService`, `IOrganisationService`, `ISieImportService`, `ISupplierInvoiceService`, `IVatReportCsvExporter`, `IVoucherGapService`, `IYearEndClosingService`, `IDocumentService`, `IDocumentProvider`) from `KoalaBooks.Application.Services` to `KoalaBooks.Domain.Interfaces`, plus one-line `using`-statement updates to all five existing controllers (`AccountsController`, `BankTransactionsController`, `FiscalYearsController`, `JournalEntriesController`, `SupplierInvoicesController`) — the exact files Agent B/C/D/H are scoped to modify.
  **Prerequisite: merge #291 to `main` before dispatching any of Agent B-I.** Otherwise every stream starts against the pre-relocation namespace and needs a follow-up fix once #291 eventually lands — churn multiplied across every parallel stream instead of paid once, upfront. Section 1.3's interface locations below describe current `main` (pre-#291) state; re-verify each interface's namespace after #291 merges before writing any `using` statement.
...
**Prerequisite (blocks all of Agent B-I): merge PR #291 to `main` first.** #291 carries #298's interface relocation (`KoalaBooks.Application.Services` → `KoalaBooks.Domain.Interfaces` for ~14 interfaces) plus `using`-statement updates to all five existing controllers — see 1.8. #291 is currently `CONFLICTING` with `main` and needs its conflicts resolved before it can land. Agent G (Reports) already shipped ahead of this being flagged, so it's exempt/retroactive; every stream from here on should wait for #291.
